...
export async function exportToPDF(options: PDFExportOptions): Promise<void> {
  ...
  const metrics = [
    ...
    ['Taxes', formatCurrency(results.taxes, currency)],
  ...
  if (results.customerMetrics && Object.keys(results.customerMetrics).length > 0) {
    checkNewPage(60);
    yPosition = addText('CUSTOMER METRICS', margin, yPosition, { fontSize: 16, fontStyle: 'bold' });
    yPosition += 10;

    const customerMetrics = [];
    if (results.customerMetrics.ltv) {
      customerMetrics.push(['Customer Lifetime Value', formatCurrency(results.customerMetrics.ltv, currency)]);
    }
    if (results.customerMetrics.paybackPeriod) {
      customerMetrics.push(['Payback Period', `${results.customerMetrics.paybackPeriod.toFixed(1)} months`]);
    }
    if (results.customerMetrics.customersNeeded) {
      customerMetrics.push(['Customers Needed', results.customerMetrics.customersNeeded.toString()]);
    }
  ...
  if (results.riskFactors && results.riskFactors.length > 0) {
    ...
    results.riskFactors.forEach((risk) => {
      const riskColor = risk.impact === 'high' ? [220, 38, 38] : risk.impact === 'medium' ? [245, 158, 11] : [34, 197, 94];
  ...
  pdf.text(`Page ${pdf.internal.getCurrentPageInfo().pageNumber}`, pageWidth - margin - 20, footerY);
